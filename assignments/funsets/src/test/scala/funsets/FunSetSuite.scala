...
class FunSetSuite extends FunSuite {
  ...
  // -----------------------------------------
    ...
    new TestSets {
      ...
      assert(exists(s, x => x > 1), "exists 4")
      assert(!exists(s, x => x > 2), "exists 5")
    }
  }// end of test
    ...
    new TestSets {
      val s = union(s1, s2) // {1,2}
      val t = map(s, x => x+10)
      assert(contains(t, 11), "map 1")
      assert(contains(t, 12), "map 2")
      assert(!contains(t, 1), "map 3")
      val t1 = map(s, x => x*x*x)
      assert(contains(t1, 1), "map 4")
      assert(contains(t1, 8), "map 5")
      assert(!contains(t1, 2), "map 6")
